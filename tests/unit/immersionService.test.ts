...
describe('immersionService', () => {
  ...
  describe('gradeImmersionAnswer', () => {
    it('awards XP directly if directXp is provided', async () => {
      ...
      const result = await gradeImmersionAnswer(
        'u1',
        {
          question: 'q',
          userAnswer: 'a',
    ...
    it('updates assignment progress if directXp and assignmentId are provided', async () => {
      vi.mocked(updateImmersionAssignmentScore).mockResolvedValueOnce({
        score: 5,
        targetScore: 10,
        passed: false
      } as any);
    ...
    it('throws error if quota exceeded', async () => {
      ...
      ).rejects.toThrow('Daily AI quota exceeded. Please try again tomorrow.');
    });
