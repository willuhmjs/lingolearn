...
${language.name === 'German' ? `CRITICAL INSTRUCTION FOR GERMAN: You MUST return the word with actual umlauts (ä, ö, ü, ß) instead of their multi-letter equivalents (ae, oe, ue, ss). For example, return "Käse" instead of "Kaese", "groß" instead of "gross".
...
Return the dictionary entry in the following JSON format:
{
  ...
  "level": "A1" | "A2" | "B1" | "B2" | "C1" | "C2" | null
		...
		if (language.name.toUpperCase() === 'GERMAN' && llmResult.lemma.charAt(0) === llmResult.lemma.charAt(0).toLowerCase()) {
		...
		const newVocab = await prisma.vocabulary.create({
			data: {
				...
				gender: llmResult.gender && ['MASCULINE', 'FEMININE', 'NEUTER'].includes(llmResult.gender.toUpperCase()) ? llmResult.gender.toUpperCase() as "MASCULINE" | "FEMININE" | "NEUTER" : null,
