...
export class CefrService {
  private static async getGateStats(
    userId: string,
    languageId: string,
    level: string
  ): Promise<GateStats> {
    const freqGateTarget =
      CEFR_CONFIG.VOCAB_FREQ_GATE[level as keyof typeof CEFR_CONFIG.VOCAB_FREQ_GATE] ?? 150;
    const totalVocabTarget =
      CEFR_CONFIG.TOTAL_VOCAB_GATE[level as keyof typeof CEFR_CONFIG.TOTAL_VOCAB_GATE] ?? 0;

    const topFreqWords = await prisma.vocabulary.findMany({
      where: { languageId, cefrLevel: level, frequency: { not: null } },
      orderBy: { frequency: 'asc' },
      take: freqGateTarget,
      select: { id: true }
    });
    const topFreqWordIds = topFreqWords.map((w) => w.id);
    ...
    const [
      ...
      totalVocabMastered,
      totalGrammarAtLevel,
      interactedGrammarCount
    ] = await Promise.all([
      prisma.userVocabulary.findMany({
        where: { userId, vocabularyId: { in: topFreqWordIds }, srsState: SrsState.MASTERED },
        select: {
          vocabularyId: true,
          eloRating: true,
          vocabulary: {
            select: {
              progress: { where: { userId }, select: { hasProduced: true } }
            }
          }
        }
      }),
      prisma.userGrammarRule.findMany({
        where: { userId, grammarRule: { languageId, level }, srsState: SrsState.MASTERED },
        select: {
          grammarRuleId: true,
          eloRating: true,
          grammarRule: {
      ...
      prisma.userVocabulary.count({
        where: { userId, vocabulary: { languageId }, srsState: SrsState.MASTERED }
      }),
      prisma.grammarRule.count({ where: { languageId, level } }),
      prisma.userGrammarRule.count({
        where: { userId, grammarRule: { languageId, level } }
      })
    ]);
    ...
    const allElos = [
      ...freqGateItems.map((v) => v.eloRating),
      ...grammarGateItems.map((g) => g.eloRating)
    ];
    const averageElo = allElos.length > 0 ? allElos.reduce((a, b) => a + b, 0) / allElos.length : 0;

    return {
      freqVocab: {
        mastered: freqGateItems.length,
        produced: producedFromTopFreq,
        target: freqGateTarget,
        required: topFreqWords.length,
        wordIds: topFreqWordIds
      },
      grammar: {
        mastered: grammarGateItems.length,
        produced: producedGrammarCount,
        totalAtLevel: totalGrammarAtLevel,
        interacted: interactedGrammarCount
      },
      totalVocab: {
        mastered: totalVocabMastered,
        target: totalVocabTarget
      },
      averageElo
    };
  }

  private static getDecayedElo(currentElo: number, cefrLevel: string): number {
    const baseline =
      CEFR_CONFIG.BASE_ELO[cefrLevel as keyof typeof CEFR_CONFIG.BASE_ELO] ??
      CEFR_CONFIG.BASE_ELO.A1;
    if (currentElo <= baseline) return currentElo;
    return currentElo - (currentElo - baseline) * CEFR_CONFIG.DECAY.RATE;
  }
  ...
  static async applyEloDecay(userId: string, languageId: string): Promise<void> {
    ...
    const vocabUpdates = staleVocab
      .map((item) => {
        ...
        const decayedElo = this.getDecayedElo(item.eloRating, item.vocabulary.cefrLevel);
        if (decayedElo === item.eloRating) return null;
    ...
    const grammarUpdates = staleGrammar
      .map((item) => {
        if (!item.grammarRule) return null;
        const decayedElo = this.getDecayedElo(item.eloRating, item.grammarRule.level);
        if (decayedElo === item.eloRating) return null;
  ...
  static async evaluateLevelUp(userId: string, languageId: string): Promise<LevelUpdate | null> {
    ...
    const stats = await this.getGateStats(userId, languageId, currentLevel);

    // --- Vocab gate: frequency-coverage ---
    // Require mastery of the top-N most frequent words at this level (by corpus rank).
    const isFreqVocabMet =
      stats.freqVocab.required > 0 && stats.freqVocab.mastered >= stats.freqVocab.required;

    // --- Total Vocab gate ---
    // Ensures a broad base across all levels (cumulative).
    const isTotalVocabMet = stats.totalVocab.mastered >= stats.totalVocab.target;
    ...
    const minGrammarInteraction = Math.min(
      stats.grammar.totalAtLevel,
      CEFR_CONFIG.GRAMMAR_MIN_INTERACTION
    );
    const grammarMastery =
      stats.grammar.interacted > 0 ? stats.grammar.mastered / stats.grammar.interacted : 0;
    const isGrammarMet =
      stats.grammar.interacted >= minGrammarInteraction &&
    ...
    const totalGateMastered = stats.freqVocab.mastered + stats.grammar.mastered;
    const totalGateProduced = stats.freqVocab.produced + stats.grammar.produced;
    ...
    const averageElo = stats.averageElo;
    const isEloMet = averageElo >= eloTarget;

    if (isFreqVocabMet && isTotalVocabMet && isGrammarMet && isEloMet && isProductionMet) {
      const vocabMastery =
        stats.freqVocab.required > 0 ? stats.freqVocab.mastered / stats.freqVocab.required : 0;
  ...
  private static async seedGrammarFsrsCheckIn(
    userId: string,
    rules: { id: string }[]
  ): Promise<void> {
    if (rules.length === 0) return;

    const AUTO_MASTERY_STABILITY = 7; // days
    const now = new Date();
    ...
    if (progressOps.length > 0) {
      ...
      console.log(
        `[Grammar Mastery] Seeded FSRS check-in records for ${progressOps.length} rule(s)`
      );
    }
  }
  ...
  static async applyGrammarMasteryForLevel(
    ...
    if (masterOps.length > 0) await prisma.$transaction(masterOps);
    ...
    await this.seedGrammarFsrsCheckIn(userId, rulesToMaster);
  ...
  static async getCefrProgress(userId: string, languageId: string): Promise<CefrProgressDetail> {
    ...
    const grammarMastery =
      stats.grammar.interacted > 0 ? stats.grammar.mastered / stats.grammar.interacted : 0;
    const grammarExposure =
      stats.grammar.totalAtLevel > 0 ? stats.grammar.interacted / stats.grammar.totalAtLevel : 0;
    const averageElo = stats.averageElo;
    ...
    const totalVocabProgress =
      stats.totalVocab.target > 0
        ? Math.min(1, stats.totalVocab.mastered / stats.totalVocab.target)
        : 1;
    ...
    const grammarInteractionProgress =
      minGrammarInteraction > 0 ? Math.min(1, stats.grammar.interacted / minGrammarInteraction) : 1;
    const grammarMasteryProgress =
      stats.grammar.interacted > 0
    ...
    return {
      ...
      nextLevel,
      percentComplete,
      freqCoverageCount: stats.freqVocab.mastered,
      freqCoverageTarget: stats.freqVocab.required,
