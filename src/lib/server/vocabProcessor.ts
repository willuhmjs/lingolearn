...
export async function upsertAiVocabEntry(
  ...
  if (!skipDbWrite) {
    ...
    // Heuristic: Map frequency rank to CEFR level
    const heuristicCefr = CefrService.mapRankToCefr(frequency);

    // Use AI-provided CEFR level if present and valid, otherwise fallback to heuristic.
    // We trust the AI more for specific nuances, but the heuristic is a robust baseline.
    const cefrLevel = v.cefrLevel && /^[ABC][12]$/.test(v.cefrLevel) ? v.cefrLevel : heuristicCefr;
          ...
          try {
            const ctxRes = await generateChatCompletion({
              ...
              messages: [
                {
                  role: 'user',
                  content: `Sentence: "${fullSentence}"\nWords to explain: ${JSON.stringify(ambiguousInSentence)}\n\nRespond with JSON in this exact shape:\n{"vocabulary":[{"lemma":"canonical dictionary base form (infinitive for verbs, nominative singular for nouns) in ${activeLangName}","meaning":"precise contextual English meaning/role in this sentence","partOfSpeech":"verb|pronoun|particle|other","cefrLevel":"A1|A2|B1|B2|C1|C2"}]}`
                }
              ],
              systemPrompt: `You are an expert ${activeLangName} lexicographer. For each word given, provide a generic, dictionary-style definition, its grammatical role, and its estimated CEFR level (A1-C2). DO NOT provide a definition that is tied to a specific context or sentence. Output ONLY valid JSON, no markdown.`,
              jsonMode: true,
              temperature: 0.1,
              onUsage: chargeQuota
                ? ({ totalTokens }) => {
